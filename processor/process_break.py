...
def label_mesh(mesh, vertex_inds):
    """Transfer vertex colors from one mesh to another"""
    # Vertices to transfer color to
    vertices = mesh.vertices[vertex_inds, :]

    # Fracture surface color is green
    mesh.visual.vertex_colors = np.repeat( np.array( [0,255,0,255] ).astype( np.uint8).reshape( 1,-1 ), \
        mesh.vertices.shape[0], 0 )

    # Label colors corresponding to exterior as red
    mesh.visual.vertex_colors[vertex_inds, :] = np.repeat( np.array( [255,0,0,255] ).astype( np.uint8 ).reshape( 1,-1 ),\
        vertices.shape[0], 0 ) 
...
def export_mesh_fragments(
    ...
    """
    Fragment the mesh into individual connected components
    with separated out fracture boundaries
    """
    face_adjacency = mesh.face_adjacency
    c_ids = trimesh.graph.connected_components(face_adjacency)
    l_c_ids = len(c_ids)
    is_break = np.zeros( l_c_ids, dtype=bool )
    centroids = np.zeros( (l_c_ids,3) )
    
    for i in range(0,l_c_ids):
        part = mesh.submesh( [c_ids[i]],append=True )
        centroids[i,:] = np.mean( part.vertices,axis=0 )
        is_break[i] = any( part.visual.vertex_colors[:,1].astype(int)==255 ) and \
            part.vertices.shape[0] > minvcount
    
    fbreak = is_break.nonzero()[0].astype(int)
    fintact = (1-is_break).nonzero()[0].astype(int)
    c_ids_break = [c_ids[i] for i in fbreak]
    c_ids_intact = [c_ids[i] for i in fintact]
    l_c_ids_b = len(c_ids_break)
    
    for i in range( 0,len(c_ids_intact) ):
        distances = np.full( l_c_ids_b, np.float32('inf') )
        for j in range( 0,l_c_ids_b ):
            distances[j] = np.sum((centroids[fintact[i],:] - centroids[fbreak[j],:]) ** 2)
        idx = distances.argmin()
        c_ids_break[idx] = np.concatenate( (c_ids_break[idx],c_ids_intact[i]) )
        
    if l_c_ids_b > 1:
        volumes = np.zeros( l_c_ids_b )
        for i in range( 0,l_c_ids_b ):
            part = mesh.submesh( [c_ids_break[i]],append=True )
            volumes[i] = part.volume
        volumess = np.sort(volumes)[::-1]
        ids_sorted = np.argsort(volumes)[::-1]
    else:
        ids_sorted = np.full( 1,0 )
        
    for i in range( 0,l_c_ids_b ):
        part = mesh.submesh( [c_ids_break[ids_sorted[i]]],append=True )
        fileout = file.replace(".obj","_fragment_" + str(i) + ".obj")
        part.export( os.path.join( root, fileout ) )
...
def breaker(
    ...
    if overwrite or not os.path.exists(f_out):
        ...
        if !export_colors:
            logging.debug("Exporting broken mesh fragments with suffix _fragmentN")
            export_mesh_fragments( tri_mesh_out_b )
    ...
    if f_restoration:
        ...
        if overwrite or not os.path.exists(f_restoration):
            ...
            if !export_colors:
                logging.debug("Exporting restoration mesh fragments with suffix _fragmentN")
                export_mesh_fragments( tri_mesh_out_r )
        else:
            return

    # Save the fracturing mesh (tool)
    if f_tool:
